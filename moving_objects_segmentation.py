...
def runOnVideo(video, max_frames):
    ...
    while True:
        ...
        if (
            read_frames == 0
        ):  # in first frame we cant detect moving objects because algorithm is based on previous frames
            ...
            for i in range(
                len(new_masks)
            ):  # iterate through every mask in current frame
                jaccards = (
                    []
                )  # list of jaccard scores for pairs of mask in current frame and every mask in previous frame
                new_mask_array = new_masks[
                    i
                ].numpy()  # converting to np.array for jaccard_score function input
                found_in_prev = False  # flag in case if in previous frame corresponding mask was not detected

                for j in range(len(prev_masks)):
                    prev_mask_array = prev_masks[
                        j
                    ].numpy()  # converting to np.array for jaccard_score function input
                    # calculating jaccard score
                    jaccards.append(
                        jaccard_score(new_mask_array, prev_mask_array, average="micro")
                ...
                if max(jaccards) >= 0.5:  # threshold for jaccard score
                    ...
                    corresponding_mask_index = jaccards.index(
                        max(jaccards)
                    )  # highest jaccard score -> corresponding mask
                    ...
                    if area_p >= area_n:  # box got smaller
                        for y in range(int(n_y1.item()), int(n_y2.item())):
                            for x in range(int(n_x1.item()), int(n_x2.item())):
                                ...
                                pixel_in_prev_mask = prev_masks[
                                    corresponding_mask_index
                                ][y][x]
