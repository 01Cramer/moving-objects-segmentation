...
def convert(outputs):  # easier access to detected objects
...
video_writer = cv2.VideoWriter('test_sample_0.mp4', fourcc=cv2.VideoWriter_fourcc(*"mp4v"),
    ...
    while True:
        ...
        if read_frames == 0:
            ...
            for i in range(len(new_masks)):  # iterate through every mask in current frame
                jaccards = []  # jaccard scores for pairs of mask in current frame and every mask in previous frame
                new_mask_array = new_masks[i].numpy()
                found_in_prev = False

                for j in range(len(prev_masks)):
                    prev_mask_array = prev_masks[j].numpy()
                    jaccards.append(jaccard_score(new_mask_array, prev_mask_array, average="micro"))

                if max(jaccards) >= 0.5:  # threshold for jaccard score
                    found_in_prev = True
                    corresponding_mask_index = jaccards.index(
                        max(jaccards))  # highest jaccard score -> corresponding mask
                    ...
                    # corresponding mask bounding box
                    ...
                    # compute area of corresponding bounding boxes
